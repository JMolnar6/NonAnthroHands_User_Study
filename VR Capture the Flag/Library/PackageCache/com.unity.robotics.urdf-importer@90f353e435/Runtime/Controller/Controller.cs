...
namespace Unity.Robotics.UrdfImporter.Control
{
    ...
    public class Controller : MonoBehaviour
    {
        ...
        void Update()
        {
           ...
           HandleJointSelection(controls[0],controls[1]);
        ...
        private Vector2 GetControls(){
            ...
            Vector2 controls = new Vector2((float) 0.0, (float) 0.0);
            if (SelectionInput1){
                controls[0] = 1;
            }
            if (SelectionInput2){
            ...
            Debug.Log("controls[0] = "+ controls[0].ToString("G"));
            return controls;
        }

        public void HandleJointSelection(float SelectionInput1, float SelectionInput2){
            SetSelectedJointIndex(selectedIndex); // to make sure it is in the valid range
            UpdateDirection(selectedIndex);

            if (SelectionInput2==1)
            {
                SetSelectedJointIndex(selectedIndex - 1);
                Highlight(selectedIndex);
            }
            else if (SelectionInput1==1)
            {
                SetSelectedJointIndex(selectedIndex + 1);
                Highlight(selectedIndex);
            }
