...
public class TentacleController : MonoBehaviour {
    // All we need to do to control the tentacle is to control the target and pole for each segment. Feel free to use math to enforce a 
    //  continuous smooth spline between the two, and to cause the baseline position/orientation of the base of the second half of the 
    //  tentacle to match the first
    ...
    public GameObject segmentBase = null;
    ...
    //  of the end-effector and pole, and then the 6DOF position/rotation recordings will be trained to match the 
    //  tentacle motion
    ...
    public void Contort(GameObject target){
    ...
    //  how the tentacle should move. Two options are given here, one for VR and one for a flat screen.
        ...
        // float base_length = 0f; //INSERTED FOR DEBUGGING PURPOSES. THE TARGET IS NO LONGER REQUIRED TO BE WITHIN THE UNIT SPHERE, SO THIS SHOULD BE IRRELEVANT ANYWAY
            ...
            if (base_length<=tentacleLength){
                ...
                Vector3 qcposition = GetQCPos(base_length, position);
    ...
    private Vector3 GatherControlsKeyboard() {
    ...
    private Vector3 GatherControlsVR(){
    ...
    private Vector3 GetQCPos(float base_length, Vector3 position){
        // Algorithm: 
        ...
        Vector3 tempPos = segmentBase.transform.position + height; // Useful if the tentacle is anchored to a moving object, i.e. the user's arm
        ...
        rotateZ.SetColumn(0, new Vector4(Mathf.Cos(angle2), Mathf.Sin(angle2), 0, 0));
        rotateZ.SetColumn(1, new Vector4(-Mathf.Sin(angle2), Mathf.Cos(angle2), 0, 0));
        rotateZ.SetColumn(2, new Vector4(0,0,1,0));
        rotateZ.SetColumn(3, new Vector4(0,0,0,1));

        // Unity uses a left-handed coordinate system. All Z column angles need to be negative
        rotateY.SetColumn(0, new Vector4(Mathf.Cos(angle1), 0, Mathf.Sin(angle1), 0));
        rotateY.SetColumn(1, new Vector4(0, 1, 0, 0));
        rotateY.SetColumn(2, new Vector4(-Mathf.Sin(angle1), 0, Mathf.Cos(angle1),0));
        rotateY.SetColumn(3, new Vector4(0,0,0,1));

        Vector3 qcposition = rotateY*rotateZ*tempPos;
        return qcposition;
    }
    private Vector3[] PositionChangeCartesian(Vector3 controls) {
    ...
    private Vector3[] PositionChangeCylindrical(Vector3 controls) {
        // Vector3 position  = spline.nodes[1].Position;
        // Vector3 direction = spline.nodes[1].Direction;
        Vector3 position = target.transform.position;
        Vector3 direction = pole.transform.position;
    ...
    private Vector3[] PositionChangeSpherical(Vector3 controls) {
        // Vector3 position  = spline.nodes[1].Position;
        // Vector3 direction = spline.nodes[1].Direction;
        Vector3 position = target.transform.position;
        Vector3 direction = pole.transform.position;
    ...
    private Vector3[] PositionChangeOdd(Vector3 controls) {
        // Vector3 position  = spline.nodes[1].Position;
        // Vector3 direction = spline.nodes[1].Direction;
        Vector3 position = target.transform.position;
        Vector3 direction = pole.transform.position;
